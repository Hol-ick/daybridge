...
시간 설정을 하지 않은 상태에서는 시작·마감 시간을 임의로 채우지 않는다. 이때는 `mode=todo`로 오늘의 작업 목록만 보여주며, 시간·점심시간·18:00 자동 종료를 사용하지 않는다. 두 시간을 모두 입력하고 저장하면 기존의 `HH:00–HH:50` 시간표 모드로 전환된다.
