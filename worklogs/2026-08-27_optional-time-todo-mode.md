...
- `timeConfigured=false`이면 스케줄러가 작업 시간·점심 배치를 만들지 않고
  무시간 `mode=todo` focus 목록을 생성한다. 근무일 카운트다운은 별도 시계로 유지한다.
...
- 목록 모드에서는 시간표 드래그 이동만 사용하지 않는다. 근무일 카운트다운과 18:00 자동 종료는 계속 사용한다.
...
- 작업 카드에 시각이 없어도 근무일 시계(`09:00`·`11:30`·`13:00`·`18:00`)와 18:00 자동 종료는 유지한다. 작업 배치 시간과 근무일 카운트다운은 서로 다른 책임으로 분리한다.
...
- `ScheduleSurface.jsx`, `NowFocusOverlay.jsx`, `ScheduleDashboard.jsx`: 목록형 표시,
  빈 시간 입력, 목록 모드에서 작업 시간 이동만 비활성화하고 근무일 카운트다운은 유지
