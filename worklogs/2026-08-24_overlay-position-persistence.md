...
- 오버레이의 큰 타이머는 로컬 근무 구간에 맞춰 오전 `09:00→11:30`, 점심 `11:30→13:00`, 오후 `13:00→18:00`의 다음 경계까지 남은 시간을 30초마다 갱신한다. 18시 이후에는 `00:00`으로 멈춘다.
- 활성 집중 블록이 없을 때 비활성 `완료` 버튼 대신 `열기` 버튼으로 관리 화면을 열도록 해 빈 상태에서도 클릭 피드백을 제공한다.
...
- 빈 상태 오버레이의 `열기` 액션도 비활성화되지 않는 것을 확인했다.
