...
- 확장 카드의 시간은 윗줄에, 작업명은 한 줄 marquee로 아랫줄에 표시해 긴 제목도 카드 폭을 넘지 않고 이동하며 읽을 수 있다. Tauri에서 HTML5 drag가 누락되는 경계를 피하도록 mouse/pointer 이동을 직접 추적해 카드 드래그를 시작·drop하고, 카드 자체는 `min-width:0`과 `overflow:hidden`으로 목록 너비를 절대 침범하지 않는다. 오버레이에서 `미룸`·`완료`·`접기` 버튼은 제거했고 `작업 추가`는 재배치·전체 시간표와 같은 하단 행으로 이동했다.
