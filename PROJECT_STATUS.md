...
- 오버레이 제목 버튼은 native drag와 분리되어 클릭 시 같은 폭의 관리 패널을 아래쪽 요약 행에서 위로 펼친다. 패널은 하단을 transform 기준으로 삼아 열릴 때 아래→위, 닫힐 때 위→아래 방향을 유지한다. 네이티브 창도 하단을 고정한 채 520px까지 확장하며, 펼침·접힘은 CSS 높이/투명도 애니메이션으로 연결한다. 패널에서는 여유 시간 블록을 제외한 실제 일정만 시간과 할 일을 가로로 크게 보여주고, 블록 완료·미룸·재배치·전체 시간표 열기를 바로 조작할 수 있다. 실제 이동 때만 드래그를 시작하며, 활성 집중 블록이 없을 때 오전에는 `09:00→11:30`, 점심에는 `11:30→13:00`, 오후에는 `13:00→18:00` 구간의 다음 경계까지 남은 시간을 `HH:MM` 형식으로 크게 표시한다. 일정 시간이 없는 빈 상태에서는 `시간표 확인` 같은 보조 문구를 숨기고 제목만 남긴다. 타이머는 30초 주기로 갱신된다.
